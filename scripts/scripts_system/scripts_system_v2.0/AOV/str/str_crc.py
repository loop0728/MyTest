...
class board_env_opt():
    ...
    def __init__(self, client_handle, case_name, case_log_path, case_run_cnt=1):
      ...
      logger.print_info("str_crc_ok:%s ,str_crc_fail:%s, uboot_prompt:%s" %(str_crc_ok, str_crc_fail, uboot_prompt))
      logger.print_info("kernel_prompt:%s suspend_crc_start_addr:%s, suspend_crc_end_addr:%s" %(kernel_prompt, suspend_crc_start_addr, suspend_crc_end_addr))
      super().__init__()
    ...
    def change_bootargs(self, suspend_crc_state):
        ...
        if self.uboot_stage == 1:
            ...
            logger.print_info("kernel_stage is %s kernel_prompt_str:%s \n" %(self.kernel_stage, kernel_prompt_str))
    ...
    def runcase(self):
        ...
        logger.print_info("uboot_prompt is %s, kernel_prompt is %s, suspend_crc_start_addr is %s, suspend_crc_end_addr is %s\n" \
