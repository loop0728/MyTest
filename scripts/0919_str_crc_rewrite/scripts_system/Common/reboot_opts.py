...
class RebootOpts():
    ...
    def __init__(self, device, timeout=30):
        ...
        self.kernel_prompt = '/ #'
        self.borad_cur_state = BootStage.E_BOOTSTAGE_UNKNOWN.name
    ...
    def _get_cur_boot_state(self):
        ...
        if self.borad_cur_state != BootStage.E_BOOTSTAGE_UNKNOWN.name:
            result = 0
        else:
            i = 1
    ...
    def check_uboot_phase(self):
        ...
        if self.borad_cur_state != BootStage.E_BOOTSTAGE_UBOOT.name:
            logger.print_warning(f"dev is not at uboot now, cur_state:{self.borad_cur_state}")
            result = 255
        return result
    ...
    def check_kernel_phase(self):
        ...
        if self.borad_cur_state != BootStage.E_BOOTSTAGE_KERNEL.name:
            logger.print_warning(f"dev is not at kernel now, cur_state:{self.borad_cur_state}")
            result = 255
        return result
