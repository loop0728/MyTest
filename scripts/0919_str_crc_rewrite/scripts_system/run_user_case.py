...
class RunUserCase:
    ...
    def parase_case_run(self) -> int:
        ...
        class_name = self.case_name.split("_")[0]  # get mode class
        ...
        if (
            ...
            instance = class_obj(
                self.case_name, self.case_run_cnt, self.module_path_name
            )  # use class creat one instance
            if not instance:
                logger.print_error("create instance fail!\n")
        else:
            instance = class_obj(
                module_name, self.case_run_cnt, self.module_path_name
            )  # use class creat one instance
            if not instance:
                logger.print_error("create instance fail!\n")
    ...
    def print_run_result(self, result):
        if result:
            logger.print_warning(
                "[AutoTest][{}][fail][{}][run_cnt:{}]".format(
                    self.case_name, result, self.case_run_cnt
                )
            )
        else:
            logger.print_warning(
                "[AutoTest][{}][pass][{}][run_cnt:{}]".format(
                    self.case_name, result, self.case_run_cnt
                )
            )
