...
"""cold reboot mode3 test scenarios"""
...
class SysappSysColdRebootMode3(RebootCase):
    ...
    def check_board_log_keyword(self)-> int:
        ...
        if not telnet.is_open:
            ...
            return 255
        telnet.write("cat /proc/kmsg")
        while wait_times < self.goto_kernel_max_time:
            result,curline = telnet.read()
    ...
    def runcase_help():
        ...
        Returns:
            None
        """
        logger.warning("support cold_reboot_mode0\1\2: 0[check result by uart\
                        send cmd is ok],1[]\n")
        logger.warning("general case runcmd:  python sysapp_run_user_case.py \
                       suite/sys/reboot/sysapp_sys_cold_reboot_mode3.py cold_reboot_mode3 1\n")
        logger.warning("support stress! eg cmd:cold_reboot_mode3_stress_5\n")
        logger.warning("stress  case runcmd:  python sysapp_run_user_case.py\
                        suite/sys/reboot/sysapp_sys_cold_reboot_mode3.py \
                       cold_reboot_mode3_stress_5 1\n")
