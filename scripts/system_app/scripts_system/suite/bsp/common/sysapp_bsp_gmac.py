...
class SysappBspGmac(SysappBspBase):
    """ Gmac class for bsp case """
    ...
    def insmod(self):
        """ Insmod func. """
        ret=sys_common.insmod_ko(self.uart,f"{self.module_path}/sstar_gmac.ko","")
        if ret is False:
    ...
    def rmmod(self):
        """ Rmmod func. """
        ret=sys_common.rmmod_ko(self.uart,f"{self.module_path}/sstar_gmac.ko")
        if ret is False:
