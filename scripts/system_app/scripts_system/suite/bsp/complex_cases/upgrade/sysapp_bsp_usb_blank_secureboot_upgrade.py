...
class SysappBspUsbBlankSecurebootUpgrade(SysappCaseBase):
    """Case for USB Blank Upgrade with secure boot."""
    ...
    def start_usb_blank_secureboot_upgrade(self):
        """start_usb_blank_secureboot_upgrade
        ...
        if ret is False:
            logger.error(f"{self.uart} [usb_blank_secureboot_upgrade]:I'm not in the special uboot")
            return ret
        else:
            logger.info("[usb_blank_secureboot_upgrade]:I'm in the special uboot")

        #autok?
        ret = self.autok_alived()
        if ret is False:
            logger.error(f"{self.uart} [usb_blank_secureboot_upgrade]:autok not alived")
            return ret
        else:
            logger.info("[usb_blank_secureboot_upgrade]:autok  alive")

        #ufu
        ret = upgrade_common.start_upgrade(self,"ufu")
        if ret is False:
            logger.error(f"{self.uart} [usb_blank_secureboot_upgrade]:Upgrade ERR")
            return ret
        else:
            logger.info("[usb_blank_secureboot_upgrade]:Upgrade Success")
        ...
        ret = self.is_in_uboot()
        if ret is False:
            logger.perror(f"{self.uart} [usb_blank_secureboot_upgrade]:ufu after not in uboot")
            return ret
        else:
            logger.info("[usb_blank_secureboot_upgrade]:ufu after in uboot")
    ...
    def runcase(self):
        ...
        """
        ret = False
        logger.info("Testing USB Blank Uprade + secureboot Image case.")
        ...
        if self.notice_prepare_hw():
            ...
            ret = self.start_usb_blank_secureboot_upgrade()
            if ret is False:
            ...
            if ret is False:
                logger.error(f"{self.uart} After ufu and change bootstrap \
                                        ,then reboot not in kernel,secuere image wrong!")
                return SysappErrorCodes.FAIL
        else:
            logger.error(f"{self.uart} This is not secureboot Image,load IPL fail or HW not ready")
            return SysappErrorCodes.FAIL

        logger.info("Testing USB Blank Uprade + secureboot Image case finish!")
    ...
    def system_help():
        ...
        Returns:
            None
        """
        logger.warning("Test for USB Blank Upgrade Secure Image.")
