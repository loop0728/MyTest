...
"""test for security boot with automatic(dualos)"""
...
class SysappBspSecurityBootAutomaticDualos(CaseBase):
    """Sysapp BSP security boot automatic dualos class
    ...
    def runcase(self):
        ...
        # 2. check if the current system is Dualos
        result = write_and_match_keyword(
            self.uart, cmd="ls /proc/dualos", keyword="/ #", read_all_data=True
        )
        if "rtos" in result:
            err_code = EC.SUCCESS
