...
from idac_var import OverdriveType, PackageType, DvfsState
from idac_var import IFORD_IDAC_VOLT_CORE_TABLE, IFORD_IDAC_VOLT_CPU_TABLE
from idac_var import IFORD_IDAC_QFN_DVFS_VCORE_TABLE
from idac_opts import IdacOpts
...
class Idac(CaseBase):
    ...
    def __init__(self, case_name, case_run_cnt=1, module_path_name='./'):
        ...
        self.case_cmd_param = {
            ...
            'cmd_cpufreq_available': (f'cat /sys/devices/system/cpu/cpufreq/policy0/'
        ...
        self.case_test_param = {
            ...
            'dvfs_on': False,
            'PackageType': PackageType.PACKAGE_TYPE_MAX,
            # parse from dts
            'kernel_base_vcore': 0,
            'kernel_base_vcpu': 0,
            'kernel_opp_table': []
    ...
    def get_package_type(self):
        ...
        Args:
            None
        Returns:
            (PackageType): return the package type of chip
        """
        result = 255
        ...
        if result == 0:
            ...
            if bit4 == 0 and bit5 == 0:
                return PackageType.PACKAGE_TYPE_QFN128
            if bit4 == 0 and bit5 == 1:
                return PackageType.PACKAGE_TYPE_BGA12
            return PackageType.PACKAGE_TYPE_BGA11
        return PackageType.PACKAGE_TYPE_MAX
    ...
    def get_ipl_volt_check_list(self, package):
        ...
        """
        for overdrive in OverdriveType:
            if overdrive != OverdriveType.OVERDRIVE_TYPE_MAX:
    ...
    def get_kernel_cpufreq_voltage_check_list(self, package):
        ...
        """
        if package == PackageType.PACKAGE_TYPE_QFN128:
            if self.case_test_param['dvfs_on']:
                self.case_target_param['cpufreq_vcore_check_list'] = (
                    IFORD_IDAC_QFN_DVFS_VCORE_TABLE[DvfsState.DVFS_STATE_OFF.value])
                self.case_target_param['cpufreq_vcpu_check_list'] = (
                    IFORD_IDAC_QFN_DVFS_VCORE_TABLE[DvfsState.DVFS_STATE_OFF.value])
            else:
                self.case_target_param['cpufreq_vcore_check_list'] = (
                    IFORD_IDAC_QFN_DVFS_VCORE_TABLE[DvfsState.DVFS_STATE_ON.value])
                self.case_target_param['cpufreq_vcpu_check_list'] = (
                    IFORD_IDAC_QFN_DVFS_VCORE_TABLE[DvfsState.DVFS_STATE_ON.value])
        else:
            self.case_target_param['cpufreq_vcore_check_list'] = (
        ...
        with open(dts_file, 'r') as file:
            for line in file:
                ...
                if core_base_volt_ready == 1 and "base_voltage" in line:
                    ...
                    core_base_volt_ready = 0
                    if self.case_test_param['package_type'] == PackageType.PACKAGE_TYPE_QFN128:
                        result = 0
                        break
    ...
    def check_opp_table(self):
        ...
        if (not self.case_test_param['dvfs_on']
                and self.case_test_param['package_type'] == PackageType.PACKAGE_TYPE_QFN128):
            result = 0
        else:
            opp_check_table = (
                IFORD_IDAC_VOLT_CPU_TABLE[self.case_test_param['package_type'].value][
                    OverdriveType.OVERDRIVE_TYPE_OD.value])
    ...
    def check_avaliable_cpufreq(self, overdrive):
        ...
        Args:
            overdrive (OverdriveType): overdrive type
        Returns:
            result (int): check success, return 0; else, return 255
        """
        result = 255
    ...
    def _idac_prepare(self):
        ...
        if self.case_test_param['package_type'] == PackageType.PACKAGE_TYPE_MAX:
