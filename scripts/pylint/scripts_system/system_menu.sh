...
{
    echo "exit server py"
    python -B sysapp_exit_server.py
    sleep 3
    final_result1=$?
...
RunCaseDebug()
{
    ...
    start_time=$(date +%s)
    python3 -B sysapp_run_user_case.py ${2} ${3} ${4}
    result=$?
    if [ $result -ne 0 ]; then
        final_result=$result
    fi
...
AddCaseSystem()
{
    ...
    if [ ! -z "${run_all_solution}" ] && [ $# -eq 6 ]; then
        if [ "${6}" = "skip" ]; then
            return
        fi
    fi
...
case_list()
{
    if [ -z "${module_list}" ]; then
        source ./cases/iford_cases/setup.sh
    else
        echo "module list: ${module_list}"
        source ./cases/iford_cases/setup.sh "${module_list}"
    fi
}
